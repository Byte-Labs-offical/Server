...
function buildPanelPayload({ guildName, iconUrl, verifiedRoleId }) {
...
function buildGuildConfigPayload({ guildName, settings, stats, iconUrl }) {
  const container = buildBaseContainer()
    .addSectionComponents(
      maybeThumbnail(
        new SectionBuilder().addTextDisplayComponents(
...
function buildSetupSavedPayload({ settings, iconUrl, guildName }) {
  const container = buildBaseContainer()
    .addSectionComponents(
      maybeThumbnail(
        new SectionBuilder().addTextDisplayComponents(
...
function buildSettingsSavedPayload({ settings, iconUrl, guildName }) {
  const container = buildBaseContainer()
    .addSectionComponents(
      maybeThumbnail(
        new SectionBuilder().addTextDisplayComponents(
...
function buildStatsPayload({ guildName, stats, iconUrl, member, challenge }) {
...
function buildResetPayload({ guildName, member, removedRole }) {
